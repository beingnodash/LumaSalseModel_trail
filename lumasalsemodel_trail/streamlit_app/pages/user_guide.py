...
- **模式B**：高校付费 + 学生分层付费（平衡的合作模式）
- **模式C**：高校免费 + 学生分层付费（适合预算受限但学生付费能力强的院校）
...
- **建议值**：8-12（对应4-6年），确保覆盖完整业务周期

#### 2. 价格参数

**学生端定价**：
- `price_per_use`：单次使用价格（3-15元）
- `price_1year_member`：1年订阅价格（100-300元）
- `price_3year_member`：3年订阅价格（250-600元）
- `price_5year_member`：5年订阅价格（400-800元）
...
**定价策略建议**：
- **差异化定价**：根据目标客户群体设置不同价格档次
- **价值导向**：价格应反映产品价值和客户获得的收益
- **竞争考虑**：参考市场竞品的价格水平
...
**分布策略**：
- **模式A**：适合预算充足的985/211高校
- **模式B**：主流合作模式，平衡各方利益
- **模式C**：适合预算紧张但学生付费能力强的高校

#### 5. 学生市场细分分布

**付费方式分布**：
- `per_use_ratio`：按次付费比例（0.2-0.6）
- `subscription_period_distribution`：订阅期限分布

**期限分布**：
- `1year`：1年订阅占比（0.4-0.8）
- `3year`：3年订阅占比（0.1-0.4）
- `5year`：5年订阅占比（0.05-0.2）
...
**学生留存**：
- `student_per_use_repurchase`：按次付费复购率（30%-90%）
- `student_subscription_renewal`：订阅续费率（60%-90%）

**提升策略**：
- **高校续费**：重点关注服务质量和ROI证明
- **学生留存**：产品体验和价值感知是关键
- **分级服务**：为不同客户提供差异化的服务等级

#### 7. 分成比例
- `luma_share_from_student`：B/C模式统一分成比例（20%-80%）

**分成策略**：
- **合理分成**：平衡Luma收益和高校积极性
- **动态调整**：可以考虑根据合作深度调整分成比例
- **价值证明**：高分成需要相应的价值支撑
...
- `luma_revenue_from_student_share`：来自学生付费的分成收入
- **分析价值**：了解收入稳定性和增长驱动力
...
Luma高校销售与收益分析模型是一个功能强大的决策支持工具，通过合理使用其各项功能，您可以：
...
希望这份详细的用户指南能帮助您更好地使用这个工具。如果在使用过程中遇到任何问题，请参考常见问题部分或联系技术支持团队。

**记住**：工具只是辅助，最终的商业决策仍需要结合您的行业经验、市场洞察和战略判断。祝您使用愉快，决策成功！

---

**版本信息**：用户指南 v2.0 | 更新日期：2024年12月 | 对应应用版本：Streamlit App v2.0
""")
else:
    # 显示选中的章节内容
...
<hr>
<small><strong>Luma高校销售与收益分析模型</strong> | 版本 v2.0 | © 2024 Luma AI</small>
</div>
""", unsafe_allow_html=True)