...
st.sidebar.markdown("""
<small>不同合作模式的占比，总和应为1.0（100%）。各模式的收入结构不同：</small>
- <small>**Type1**: 基础合作模式，有固定接入费</small>
- <small>**Type2**: 增强合作模式，有不同级别(a/b/c)</small>
- <small>**Type3**: 高级合作模式，收入最高</small>
""", unsafe_allow_html=True)

col1, col2 = st.sidebar.columns(2)
with col1:
    type1_share = st.number_input(
        "Type1占比", 
        0.0, 1.0, 0.2, 
        format="%.2f",
        help="Type1合作模式的占比，范围0-1（相当于0-100%）"
    )
    type2a_share = st.number_input(
        "Type2a占比", 
        0.0, 1.0, 0.0, 
        format="%.2f",
        help="Type2a合作模式的占比，范围0-1"
    )
    type2b_share = st.number_input(
        "Type2b占比", 
        0.0, 1.0, 0.0, 
        format="%.2f",
        help="Type2b合作模式的占比，范围0-1"
    )
with col2:
    type2c_share = st.number_input(
        "Type2c占比", 
        0.0, 1.0, 0.2, 
        format="%.2f",
        help="Type2c合作模式的占比，范围0-1"
    )
    type3_share = st.number_input(
        "Type3占比", 
        0.0, 1.0, 0.6, 
        format="%.2f",
        help="Type3合作模式的占比，范围0-1"
    )

# 计算总和并显示警告
mode_sum = type1_share + type2a_share + type2b_share + type2c_share + type3_share
if not np.isclose(mode_sum, 1.0):
    st.sidebar.warning(f"模式分布总和为 {mode_sum:.2f}，应为 1.0（100%）")
else:
    st.sidebar.success("模式分布总和正确！")
...
renewal_rate_uni = st.sidebar.slider(
    "高校年度续约率", 
    0.0, 1.0, 0.8, 
    format="%.2f",
    help="高校客户每年续约的概率。例如，0.8表示80%的高校客户会续约。"
)

renewal_rate_student = st.sidebar.slider(
    "学生年度续约率", 
    0.0, 1.0, 0.7, 
    format="%.2f",
    help="学生每年续约的概率。例如，0.7表示70%的学生会续约。"
)
...
with st.sidebar.expander("高级参数设置（点击展开）"):
    ...
    col1, col2 = st.columns(2)
    with col1:
        type1_access_fee = st.number_input(
            "Type1接入费(元)", 
            min_value=0.0, 
            max_value=500000.0, 
            value=200000.0, 
            step=10000.0,
            format="%.1f",
            help="Type1模式下高校支付的一次性接入费。默认为200,000元。"
        )
        type2a_access_fee = st.number_input(
            "Type2a接入费(元)", 
            min_value=0.0, 
            max_value=500000.0, 
            value=200000.0, 
            step=10000.0,
            format="%.1f",
            help="Type2a模式下高校支付的一次性接入费。默认为200,000元。"
        )
    with col2:
        type2b_access_fee = st.number_input(
            "Type2b接入费(元)", 
            min_value=0.0, 
            max_value=500000.0, 
            value=150000.0, 
            step=10000.0,
            format="%.1f",
            help="Type2b模式下高校支付的一次性接入费。默认为150,000元。"
        )
        type2c_access_fee = st.number_input(
            "Type2c接入费(元)", 
            min_value=0.0, 
            max_value=500000.0, 
            value=100000.0, 
            step=10000.0,
            format="%.1f",
            help="Type2c模式下高校支付的一次性接入费。默认为100,000元。"
        )
    
    # 分成比例参数
    st.markdown("##### Luma分成比例")
    col1, col2 = st.columns(2)
    with col1:
        type2a_luma_share = st.slider(
            "Type2a Luma分成比例", 
            0.0, 1.0, 0.5, 
            format="%.2f",
            help="Type2a模式下Luma从学生付费中获得的比例。例如，0.5表示Luma获得50%，高校获得50%。"
        )
        type2b_luma_share = st.slider(
            "Type2b Luma分成比例", 
            0.0, 1.0, 0.6, 
            format="%.2f",
            help="Type2b模式下Luma从学生付费中获得的比例。例如，0.6表示Luma获得60%，高校获得40%。"
        )
    with col2:
        type2c_luma_share = st.slider(
            "Type2c Luma分成比例", 
            0.0, 1.0, 0.7, 
            format="%.2f",
            help="Type2c模式下Luma从学生付费中获得的比例。例如，0.7表示Luma获得70%，高校获得30%。"
        )
...
if st.session_state.model_results is not None:
    ...
    with tab2:
        ...
        if mode_columns:
            ...
            st.markdown("""
            <small>**合作模式说明**：</small>
            - <small>**Type1**：高校支付固定接入费，学生付费全部归高校</small>
            - <small>**Type2a/b/c**：高校支付固定接入费，Luma与高校按比例分成学生付费</small>
            - <small>**Type3**：无固定接入费，学生付费全部归Luma</small>
            """, unsafe_allow_html=True)
        else:
